...
def _maximum(image, mask, weights, info, axis):
    ...
    Parameters:
        ...
        axis (tuple of ints): Tuple of axes over which to compute the maximum.
    ...
    if image.dtype.kind == 'b':
        ...
        # If the minimum is at (or below) the type's fill value, `minval - 1` would
        # overflow. Promote to int64 first when possible; otherwise fall back to using
        # `minval` itself as the flag.
...
def maximum_filter(image, footprint, *, mask=None, maskval=None, weights=None,
