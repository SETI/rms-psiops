...
from psiops._utils import (_apply_op_as_filter, _check_axis, _check_image, _check_return,
                           _pixel_area, _use_shortcuts)


def maximum(image, mask=None, *, maskval=None, weights=None, nans=False, axis=None,
    ...
    Returns:
        (array or tuple): `max_image` or (`max_image`[, `new_mask`][, `new_weights`]):

        * `max_image` (array): The maximum image array, with the same dtype as `image`. If
        ...
        * `new_mask` (array): The new mask array, True wherever all the image pixels
          along the specified `axis` are masked. By default, this is returned if `mask` is
...
def _maximum(image, mask, weights, info, axis):
...
def maximum_filter(image, footprint, *, mask=None, maskval=None, weights=None, nans=False,
                   returns=None):
    ...
    Returns:
        ...
        * `max_image` (array): The maximum-filtered image array, with the same shape and
          dtype as `image`. If `image` is a MaskedArray, this will also be a MaskedArray.
