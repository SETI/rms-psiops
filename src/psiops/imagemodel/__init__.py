...
and rotate parameters, combined with an optional Scaling. The fit() method can be used
to determine the optimal transformation and Scaling to match the model to an image or
portion of an image. This can be used for applications such as PSF astrometry and
removing glare.
"""


class ImageModel:
    """Abstract class that defines a 2-D pattern that can be re-sampled into an
    arbitrary grid of pixels, given an origin, expansion factor and rotation angle.

    The integrated "volume" under an ImageModel is unchanged during these
    transformations.
    """
    ...
    def transform(self, shape, center, expand=1., rotate=0.):
        ...
        Raises:
            NotImplementedError: Always, since this is an abstract method that must be
