...
def minimum(image, mask=None, *, maskval=None, weights=None, nans=False, axis=None,
    ...
    Returns:
        (array or tuple): `min_image` or (`max_image`[, `new_mask`][, `new_weights`]):

        * `min_image` (array): The minimum image array, with the same dtype as `image`. If
...
def minimum_filter(image, footprint, *, mask=None, maskval=None, weights=None, nans=False,
                   returns=None):
    ...
    Returns:
        array or tuple: `min_image` or (`min_image`[, `new_mask`][, `new_weights`]):

        * `min_image` (array): The minimum-filtered image array, with the same shape and
