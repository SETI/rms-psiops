##########################################################################################
# image_ops/tests/test_maximum.py
##########################################################################################
...
class Test_maximum(unittest.TestCase):
    ...
    def runTest(self):
        ...
        for dtype in ('bool', 'uint8', 'int8', 'uint16', 'int16', 'uint32', 'int32',
            ...
            test = maximum(image)
            self.assertTrue(np.all(test == np.max(image,axis=0)))
            self.assertEqual(test.dtype, image.dtype)

            if image.dtype.kind == 'i' and dtype != 'int64':
                minval = np.ma.maximum_fill_value(image)
                image[0,0,0] = minval
                self.assertTrue(np.all(maximum(image) == np.max(image,axis=0)))

        image = np.random.rand(5,4,3,10,10)
        self.assertTrue(np.all(maximum(image) == np.max(image, axis=(0,1,2))))
        self.assertTrue(np.all(maximum(image, axis=1) == np.max(image, axis=1)))
        self.assertTrue(np.all(maximum(image, axis=(0,-1)) == np.max(image, axis=(0,2))))
...
class Test_maximum_filter(unittest.TestCase):
    ...
    def runTest(self):
        ...
        for i in range(100):
            ...
            for j in range(100):
                ...
                self.assertLess(np.abs(a[i,j] - np.max(values)), 1.e-14)
