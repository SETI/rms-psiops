...
def median(image, mask=None, *, maskval=None, weights=None, nans=False, axis=None,
           keepdims=False, factors=None, omit=0, returns=None):
    ...
    Parameters:
        ...
        omit (int, optional): The index identifying extreme pixels to ignore at one end of
            the distribution before taking the median. A positive value indicates the
            number to exclude at the lower end of the sorted list of pixel values along
            the selected axes; a negative value indicates the number to exclude at the
            upper end of the sorted list.
    ...
    Returns:
        (array or tuple): `median_image` or
            (`median_image`[, `new_mask`][, `new_weights`]):

        * `median_image` (array): The floating-point median image array. If `image` is a
        ...
        * `new_weights` (array): The weight array, equal to the sum of the weights of the
          elements that contributed to each pixel in `median_image`. By default, this is
...
def _median(image, mask, weights, axis, factors, omit, info):
    ...
    if factors is None and omit == 0 and _use_shortcuts():
        ...
        if weights is None:
            ...
            new_weights = info.area_factor - np.sum(mask, axis=axis)
...
def median_filter(image, footprint, *, mask=None, maskval=None, weights=None, nans=False,
                  omit=0, returns=None):
    """Filter this image such that each new pixel is equals the median over the specified
    footprint.
    ...
    Returns:
        ...
        * `median_image` (array): The floating-point median-filtered image array. If
          `image` is a MaskedArray, this will also be a MaskedArray. If `maskval` is
          specified, any masked pixels in this array will be filled with this value.
